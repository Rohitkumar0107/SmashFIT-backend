...
const ACCESS_SECRET =
  process.env.ACCESS_TOKEN_SECRET || "smashfit_dev_access_key_123";
const REFRESH_SECRET =
  process.env.REFRESH_TOKEN_SECRET || "smashfit_dev_refresh_key_456";
// [NEW] Reset password token ke liye ek alag secret bana diya for extra security
const RESET_SECRET =
  process.env.RESET_TOKEN_SECRET || "smashfit_dev_reset_key_789";
...
export const generateResetToken = (userId: string | number) => {
  ...
  return jwt.sign({ id: userId, purpose: "RESET" }, RESET_SECRET, {
    expiresIn: "15m",
  });
};
